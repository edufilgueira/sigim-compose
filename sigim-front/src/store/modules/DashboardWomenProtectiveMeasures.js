...
import {
  ...
  getQtdByRiskLevel,
  getQtdByAuthorizationMonth,
} from "../../service/DashboardWomenProtectiveMeasuresApi";
...
const mutations = {
  ...
  setQtdByAuthorizationMonthMutation: (state, value) => {
    state.qtdByAuthorizationMonth = value;
  },
};
...
const actions = {
  ...
  loadQtdByAuthorizationMonth: ({ commit }, params) => {
    getQtdByAuthorizationMonth(params)
      .then((response) => {
        commit("setQtdByAuthorizationMonthMutation", response.data);
      })
      .catch((e) => {
        Vue.$toast.error(e.response.data.error);
      });
