...
import {
  ...
  getQtdByRiskLevel,
  getQtdByOpeningMonth,
} from "../../service/DashboardWomenLawsuitsApi";
...
const mutations = {
  ...
  setQtdByOpeningMonthMutation: (state, value) => {
    state.qtdByOpeningMonth = value;
  },
};
...
const actions = {
  ...
  loadQtdByOpeningMonth: ({ commit }, params) => {
    getQtdByOpeningMonth(params)
      .then((response) => {
        commit("setQtdByOpeningMonthMutation", response.data);
      })
      .catch((e) => {
        Vue.$toast.error(e.response.data.error);
      });
