<template>
  <div class="font-system relative">
    <label
      ...
      class="absolute pt-4 pb-3 px-12 opacity-75 cursor-text"
      :for="id"
    >
      {{ labelText }}
    </label>
    <button type="button" :class="{ 'cursor-default': !hiddenValue }" @click.stop="showPassword">
      <AppMaterialIcon :iconName="icon" :iconType="iconType" class="icon-left" :class="[textColorClass]" />
    </button>
    <input
      :id="id"
      :type="typeInput"
      :class="[textColorClass, borderColorClass]"
      class="border focus:outline-none pt-4 pb-3 pl-5 pr-12 w-full bg-white"
      @focus="moveLabelUp"
      @blur="moveLabelDown"
      v-model="valueInput"
      @input="onInput()"
