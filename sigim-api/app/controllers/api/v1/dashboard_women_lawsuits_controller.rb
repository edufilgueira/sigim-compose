class Api::V1::DashboardWomenLawsuitsController < ApplicationController
  ...
  # QUANTIDADE TOTAL
  def qtd_total
    if current_api_user.can?(:dashboard_women_lawsuit_index)
      @result = Lawsuit
        .joins(:victim)
  ...
  def qtd_total_by_opening_month
    if current_api_user.can?(:dashboard_women_lawsuit_index)
      @result = Lawsuit
        .joins(:victim)
        .select("
          EXTRACT (MONTH FROM (lawsuits.opening_date)) AS month, 
          COUNT(people.id) AS total
        ")
        .group("
          month
      ...
      if params[:source_system_id].present? && params[:source_system_id].to_s.strip != ""
        @result = @result.where("lawsuits.source_system_id = ?", params[:source_system_id])
      end

      if params[:opening_date_year].present? && params[:opening_date_year].to_s.strip != ""
        @result = @result.where("EXTRACT (YEAR FROM (lawsuits.opening_date)) = ?", params[:opening_date_year])
      end

      if params[:opening_date_month].present? && params[:opening_date_month].to_s.strip != ""
        @result = @result.where("EXTRACT (MONTH FROM (lawsuits.opening_date)) = ?", params[:opening_date_month])
      end
